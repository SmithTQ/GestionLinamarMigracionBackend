...
class ProductCatalogController extends Controller
{
    ...
    #[OA\Post(path: '/api/v1/products', operationId: 'createProduct', tags: ['Productos'], summary: 'Crear producto', responses: [new OA\Response(response: 201, description: 'Producto creado')])]
    public function storeProduct(StoreProductRequest $request): JsonResponse
    {
        return response()->json(['codigo' => 201, 'mensaje' => 'Producto creado.', 'datos' => new ProductResource(Product::create($request->validated()))], 201);
    }
    #[OA\Patch(path: '/api/v1/products/{product}', operationId: 'updateProduct', tags: ['Productos'], summary: 'Actualizar producto', responses: [new OA\Response(response: 200, description: 'Producto actualizado')])]
    public function updateProduct(UpdateProductRequest $request, int $product): JsonResponse
    {
        $item = Product::findOrFail($product);
        $item->update($request->validated());
        return response()->json(['codigo' => 200, 'mensaje' => 'Producto actualizado.', 'datos' => new ProductResource($item->fresh('subcategory'))]);
    }
    ...
    public function campaignProducts(Request $request, int $campaign): JsonResponse
    {
        ...
        $products = $item->products()->where('products.is_active', true)->when($request->has('is_available'), fn($query) => $query->wherePivot('is_available', $request->boolean('is_available')), fn($query) => $query->wherePivot('is_available', true))->when($request->filled('search'), fn($query) => $query->where(fn($sub) => $sub->where('products.name', 'like', '%'.$request->string('search')->toString().'%')->orWhere('products.sku', 'like', '%'.$request->string('search')->toString().'%')))->when($request->filled('subcategory_id'), fn($query) => $query->where('products.subcategory_id', $request->integer('subcategory_id')))->tap(function ($query) use ($request): void {
    ...
    public function assignCampaignProducts(AssignCampaignProductsRequest $request, int $campaign): JsonResponse
    {
        ...
        return response()->json(['codigo' => 200, 'mensaje' => 'Productos de campaña actualizados.', 'datos' => ProductResource::collection($item->products()->with('subcategory.category')->orderByPivot('sort_order')->get())]);
    }
}
