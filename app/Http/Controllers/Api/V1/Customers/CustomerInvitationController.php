...
class CustomerInvitationController extends Controller
{
    ...
    {
        $items = Customer::query()->where('is_active', true)->when($request->filled('search'), fn ($q) => $q->where(fn ($sub) => $sub->where('full_name', 'like', '%'.$request->string('search').'%')->orWhere('whatsapp_number', 'like', '%'.$request->string('search').'%')->orWhere('email', 'like', '%'.$request->string('search').'%')))->tap(fn ($q) => $this->applySorting($q, $request, ['full_name' => 'full_name', 'whatsapp_number' => 'whatsapp_number', 'email' => 'email', 'created_at' => 'created_at'], 'full_name'))->paginate(min($request->integer('per_page', 20), 100));
    ...
    public function showCustomer(int $customer): JsonResponse
    {
        $item = Customer::with(['orders' => fn ($q) => $q->latest()])->findOrFail($customer);
    ...
    {
        $item = FormInvitation::findOrFail($invitation);
