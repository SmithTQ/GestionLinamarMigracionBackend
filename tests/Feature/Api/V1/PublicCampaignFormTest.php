...
class PublicCampaignFormTest extends TestCase
{
    ...
    public function test_public_form_returns_catalogs_and_creates_idempotent_order(): void
    {
        ...
        $campaign = Campaign::create(['code' => 'PUBLIC-CAMP', 'name' => 'Campana publica', 'status' => 'open']);
        $campaign->branches()->attach($branch);
        $district = District::create(['code' => 'LIM-MIR', 'name' => 'Miraflores']);
        $districtList = DistrictList::create(['code' => 'PUBLIC-LIST', 'name' => 'Cobertura publica', 'is_active' => true]);
        $districtList->districts()->attach($district);
        $campaign->districtLists()->attach($districtList);
        $product = Product::create(['sku' => 'PUB-001', 'name' => 'Producto publico', 'slug' => 'producto-publico', 'base_price' => 20]);
